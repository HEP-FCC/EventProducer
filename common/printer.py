...
class Printer:
    ...
    def __init__(
        self, yaml_dir: str, webfile_path: str, para: ModuleType, matching: bool
    ):
        ...
        if os.environ.get("KEY4HEP_STACK") is None:
            raise Exception(
                """The KEY4HEP_STACK env variable is missing.
                Please set it to the path of the key4hep stack that was used to generate the events.
                For example with `export KEY4HEP_STACK=/the/path/setup.sh`."""
            )
    ...
    def run(self):
        ...
        for process_name in process_names:
            ...
            mergefile_path = os.path.join(self.yaml_dir, process_name, "merge.yaml")
            ...
            sample_files = [
                ...
                if (
                    f.endswith(".stdhep.gz")
                    or f.endswith(".root")
                    or f.endswith(".lhe.gz")
                )
            ]
            # Remove not files
            sample_files = [
            ...
            if not self.matching and not ispythiaonly:
                ...
                cmd = (
                    "%s,,%s,,%s,,%s%i%s,,%i,,%s%i%s,,%.2f,,%s,,%s,,%s,,%s,,%s,,%s\n"
                    % (
                        process_name,
                        self.comma_me(str(events_tot)),
            ...
            process_info["key4hep-stack"] = os.environ["KEY4HEP_STACK"]
